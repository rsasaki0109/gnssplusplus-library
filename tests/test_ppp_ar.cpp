...
TEST(PPPArTest, ReducedDdRowFloorRequiresAValidPriorHold) {
    ppp_shared::PPPConfig config;
    config.clas_ar_held_minimum_dd_rows = 4;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, false), 6);
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true), 4);
    config.clas_ar_held_maximum_publication_streak = 1;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 1), 6);

    config.clas_ar_held_maximum_publication_streak = 5;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 1), 4);
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 4), 4);
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 5), 6);

    config.clas_ar_held_maximum_publication_streak = 0;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 1), 6);
    config.clas_ar_held_maximum_publication_streak = 99;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, 1), 6);
    config.clas_ar_held_maximum_publication_streak = 1;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true, -1), 6);

    config.clas_ar_held_minimum_dd_rows = 0;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true), 2);
    config.clas_ar_held_minimum_dd_rows = 99;
    EXPECT_EQ(ppp_ar::clasArMinimumDdRowsForEpoch(config, true), 6);
}
...
TEST(PPPArTest, DirectStateDdReducedRowFloorMustBeExplicit) {
    ppp_shared::PPPConfig config;
    ...
    state.covariance =
        MatrixXd::Identity(state.total_states, state.total_states) * 1e-6;

    const std::vector<SatelliteId> satellites = {
        {GNSSSystem::GPS, 1}, {GNSSSystem::GPS, 2},
        {GNSSSystem::GPS, 3}, {GNSSSystem::GPS, 101},
        {GNSSSystem::GPS, 102}, {GNSSSystem::GPS, 103},
    };
    ppp_ar::EligibleAmbiguities eligible;
    std::map<SatelliteId, ppp_shared::PPPAmbiguityInfo> ambiguity_states;
    for (int i = 0; i < static_cast<int>(satellites.size()); ++i) {
        const int state_index = state.amb_index + i;
        const auto satellite = satellites[static_cast<size_t>(i)];
        eligible.satellites.push_back(satellite);
        eligible.state_indices.push_back(state_index);
        eligible.scales.push_back(0.19);
        state.ambiguity_indices[satellite] = state_index;
        state.state(state_index) =
            (i < 3 ? static_cast<double>(i)
                   : 10.0 + static_cast<double>(i - 3)) *
            0.19;
        ambiguity_states[satellite].ambiguity_scale_m = 0.19;
    }
    const std::map<SatelliteId, double> elevations = {
        {{GNSSSystem::GPS, 1}, 40.0 * M_PI / 180.0},
        {{GNSSSystem::GPS, 2}, 45.0 * M_PI / 180.0},
        {{GNSSSystem::GPS, 3}, 50.0 * M_PI / 180.0},
    };

    const auto mrtklib_floor = ppp_ar::resolveWlnlFix(
        config, state, state.covariance, ambiguity_states, eligible,
        ppp_ar::WlnlNlInfoProvider{}, false, &elevations);
    EXPECT_FALSE(mrtklib_floor.fixed);
    EXPECT_EQ(mrtklib_floor.nb, 0);

    config.clas_ar_minimum_dd_rows = 4;
    const auto held_floor = ppp_ar::resolveWlnlFix(
        config, state, state.covariance, ambiguity_states, eligible,
        ppp_ar::WlnlNlInfoProvider{}, false, &elevations);
    EXPECT_TRUE(held_floor.fixed);
    EXPECT_EQ(held_floor.nb, 4);
    EXPECT_EQ(held_floor.hold_constraints.size(), 4u);
}
