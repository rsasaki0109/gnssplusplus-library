            ...
            config_toml.write_text(
                "\n".join(
                    [
                        ...
                        'host = "127.0.0.1"',
                        "port = 0",
                        f'port_file = "{port_file.as_posix()}"',
                        f'root = "{temp_root.as_posix()}"',
                        f'lib_pos = "{lib_pos.as_posix()}"',
                        f'rtklib_pos = "{rtklib_pos.as_posix()}"',
                        f'odaiba_summary = "{summary_json.as_posix()}"',
                        f'rcv_status = "{status_json.as_posix()}"',
                        f'artifact_manifest = "{artifact_manifest.as_posix()}"',
                        f'docs_url = "{docs_url}"',
                    ]
                )
                + "\n",
