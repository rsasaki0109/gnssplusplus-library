...
TEST(TightCouplingProcessorTest, CanContinueIncrementallyWithoutGnssReanchor) {
    TightCouplingProcessor::Config config;
    config.lever_arm_body = {0.3, 0.0, 0.5};
    config.zupt_enable = false;
    TightCouplingProcessor processor(config);
    ...
    ASSERT_TRUE(processor.reanchor(antenna, Eigen::Matrix3d::Identity(),
                                   ...
                                   bootstrap.nominal.time, &bootstrap));
    for (int i = 1; i <= 20; ++i) {
        processor.processImuSample(stationarySample(100.0 + i * 0.01));
    }
    ASSERT_TRUE(processor.prepareTimeUpdate().valid);
    ASSERT_TRUE(processor.continueFromPrediction());
    for (int i = 21; i <= 40; ++i) {
        processor.processImuSample(stationarySample(100.0 + i * 0.01));
    }
    const auto second_update = processor.prepareTimeUpdate();
    ASSERT_TRUE(second_update.valid);
    EXPECT_LT(second_update.antenna_delta_ecef.norm(), 1e-8);
    EXPECT_EQ(processor.diagnostics().prediction_continuations, 1u);
}
