...
bool buildClasGridTideInterpolation(
    ...
    ppp_clasnat_trop::ClasGridReference reference;
    if (!ppp_clasnat_trop::resolveClasGridReference(
...
int preferredClasNetworkId(const std::map<std::string, std::string>& atmos_tokens) {
    int network_id = 0;
    if (!ppp_clasnat_trop::parseAtmosTokenInt(atmos_tokens, "atmos_network_id", network_id)) {
        return 0;
    }
...
std::map<std::string, std::string> selectClasEpochAtmosTokens(
    ...
    for (const auto& satellite : satellites) {
        ...
        for (const auto& correction : sat_it->second) {
            ...
            ppp_clasnat_trop::ClasGridReference grid_reference;
            const bool has_grid = ppp_clasnat_trop::resolveClasGridReference(
...
CLASEpochContext prepareClasnatEpochContext(
    ...
    context.osr_corrections = computeOsrClasnat(
        obs,
        nav,
        ssr,
        context.epoch_atmos_tokens,
...
std::vector<OSRCorrection> computeOsrClasnat(
    ...
    ppp_clasnat_trop::parseAtmosTokenInt(
    ...
    for (const auto& sat : obs.getSatellites()) {
        ...
        if (!iono_atmos_tokens.empty()) {
            const double stec_tecu = ppp_clasnat_trop::atmosphericStecTecu(
                iono_atmos_tokens,
                sat,
                receiver_pos,
                config.clas_expanded_value_construction_policy,
