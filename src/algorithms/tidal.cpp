...
void nutationIau1980(double /*t*/, const double* /*f*/, double& dpsi, double& deps) {
    // The nutation terms rotate the lunar vector by tens of kilometres, which
    // is below 0.1 mm in the final displacement.  CLAS parity is dominated by
    // the tide model, UT1, BLQ, and pole terms, so keep this intentionally
    // lightweight instead of carrying the 106-row IAU 1980 table here.
    dpsi = 0.0;
    deps = 0.0;
}
