...
bool resolveClasGridReference(const std::map<std::string, std::string>& atmos_tokens,
    ...
    dlat_deg = receiver_lat_deg - nearest->latitude_deg;
    dlon_deg = receiver_lon_deg - nearest->longitude_deg;
    residual_index = static_cast<size_t>(std::max(nearest->grid_no - 1, 0));
    return true;
}
...
double atmosphericTroposphereCorrectionMeters(
    ...
    const bool have_grid_reference =
        resolveClasGridReference(atmos_tokens, receiver_position, dlat_deg, dlon_deg, residual_index);
    int trop_type = -1;
    ...
    if (have_grid_reference &&
        parseAtmosListValueAtIndex(atmos_tokens, "atmos_trop_residuals_m", residual_index, value) &&
        std::isfinite(value)) {
        correction_m += value;
        have_correction = true;
    ...
    if (trop_grid_type == 1 && std::isfinite(elevation) && elevation > 0.0) {
        ...
        if (have_grid_reference &&
            parseAtmosListValueAtIndex(atmos_tokens, "atmos_trop_hs_residuals_m", residual_index, value) &&
            std::isfinite(value)) {
            hydro_delta_m = value;
            have_grid_trop = true;
        ...
        if (have_grid_reference &&
            parseAtmosListValueAtIndex(atmos_tokens, "atmos_trop_wet_residuals_m", residual_index, value) &&
            std::isfinite(value)) {
            wet_delta_m = value;
            have_grid_trop = true;
...
double atmosphericStecTecu(const std::map<std::string, std::string>& atmos_tokens,
    ...
    if (have_grid_reference && stec_type > 1 &&
        ...
        std::isfinite(value)) {
        stec_tecu += value * dlat_deg * dlon_deg;
        have_correction = true;
    }
    if (have_grid_reference && stec_type > 2) {
        if (parseAtmosTokenDouble(atmos_tokens, "atmos_stec_c02_tecu_per_deg2" + suffix, value) &&
            std::isfinite(value)) {
            stec_tecu += value * dlat_deg * dlat_deg;
            have_correction = true;
        }
        if (parseAtmosTokenDouble(atmos_tokens, "atmos_stec_c20_tecu_per_deg2" + suffix, value) &&
            std::isfinite(value)) {
            stec_tecu += value * dlon_deg * dlon_deg;
            have_correction = true;
        }
    }
    if (have_grid_reference &&
        parseAtmosListValueAtIndex(atmos_tokens, "atmos_stec_residuals_tecu" + suffix, residual_index, value) &&
        std::isfinite(value)) {
        stec_tecu += value;
        have_correction = true;
        ...
        if (ssr_products_loaded_) {
            ...
            if (ssr_products_.interpolateCorrection(
                ...
                const double trop_correction_m =
                    atmosphericTroposphereCorrectionMeters(
                        atmos_tokens,
                        receiver_position,
                        time,
                ...
                const double stec_tecu =
                    atmosphericStecTecu(atmos_tokens, observation.satellite, receiver_position);
                const double ionosphere_correction_m = observationIonosphereDelayMeters(
                    eph,
                    observation.primary_signal,
                    observation.secondary_signal,
                    ppp_config_.use_ionosphere_free,
