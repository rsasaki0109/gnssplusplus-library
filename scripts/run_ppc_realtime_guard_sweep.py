...
def run_profile(args: argparse.Namespace, profile: GuardProfile) -> tuple[Path, Path, dict[str, Any]]:
    ...
    if not matrix_json.exists():
        raise SystemExit(f"{profile.name}: missing matrix JSON: {matrix_json}")
    payload = json.loads(matrix_json.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise SystemExit(f"{profile.name}: matrix JSON must contain an object")
    return matrix_json, matrix_md, payload
...
def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Profile | Official | Delta | Positioning delta vs demo5 | P95H delta vs demo5 | Max wall | Min realtime | Min epoch rate | Args |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for profile in payload["profiles"]:
        ...
        lines.append(
            ...
            f"{fmt(profile.get('avg_positioning_delta_vs_rtklib_pct'), ' pp')} | "
            f"{fmt(profile.get('avg_p95_h_delta_vs_rtklib_m'), ' m')} | "
...
def main() -> int:
    ...
    write_outputs(args, build_payload(results))
    return 0
