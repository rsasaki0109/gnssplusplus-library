#!/usr/bin/env python3
"""Paired PPP run with the IERS Conventions 2010 HARDISP ocean-tide-
loading opt-in flag toggled, summarizing the per-epoch displacement
between the two solutions.

This is the truth-bench harness for Phase A-2b (HARDISP). It runs
``gnss_ppp`` twice on the same input observations / navigation /
products — once with ``--no-iers-ocean-loading`` (the legacy
11-constituent direct cosine sum that uses Unix-epoch as the
astronomical phase reference, today's default) and once with
``--use-iers-ocean-loading`` (the IERS Conventions 2010 §7.1.2
HARDISP spline-interpolated 342-harmonic admittance, opt-in via the
libgnss::iers wrapper) — and reports the displacement statistics
between the two .pos outputs.

Both runs MUST be supplied the same BLQ file (``--blq``) and station
name (``--ocean-loading-station``). The harness asserts both inputs
are present.

Typical use:

    python apps/commands/benchmarks/gnss_ppp_iers_ocean_loading_bench.py \\
        --obs path/to/rover.obs \\
        --nav path/to/nav.rnx \\
        --sp3 path/to/orbit.sp3 \\
        --clk path/to/clock.clk \\
        --blq path/to/station.blq \\
        --ocean-loading-station TSKB \\
        --output-dir output/iers_ocean_loading_bench \\
        --max-epochs 600

The script writes:
    <output-dir>/legacy.pos       — gnss_ppp output with the legacy
                                    ocean-loading direct sum
    <output-dir>/iers.pos         — gnss_ppp output with HARDISP
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=ROOT_DIR / "output" / "iers_ocean_loading_bench",
    ...
    parser.add_argument(
        ...
        help="Optional acceptance gate: fail if the maximum per-epoch "
             ...
             "meters. The HARDISP path differs from the legacy direct "
             "sum by at most ~3 cm at coastal stations and ~1 cm "
             "inland, so values in the 0.05-0.20 m range are reasonable "
             "upper bounds for sanity checking.")
...
def read_pos_records(path: Path) -> list[dict[str, float | int]]:
    ...
    ``apps/commands/positioning/gnss_ppp_kinematic_signoff.py`` so the field semantics are
    consistent."""
...
def run_ppp(
    ...
    cmd += ["--use-iers-ocean-loading" if use_iers else "--no-iers-ocean-loading"]
...
def main() -> int:
    ...
    if args.blq is None:
        sys.stderr.write(
            "Error: --blq is required by the ocean-loading bench. The two "
            "paired runs must consume the same BLQ file.\n")
        return 2
    ensure_input_exists(args.blq, "BLQ ocean-loading file", ROOT_DIR)
