...
The preceding tiers use the same invariant. Score the resulting six paths with
the common metric implementation:

```bash
python3 scripts/summarize_fgo_ppc_matrix.py \
  --dataset-root data/PPC-Dataset \
  --pos tokyo/run1=output/tokyo1_selected_quality_tcm3_tier3_truthfree.pos \
  --pos tokyo/run2=output/tokyo2_selected_quality_rtkbaseline_tier2_truthfree.pos \
  --pos tokyo/run3=output/tokyo3_selected_quality_rtkbaseline_tier2_truthfree.pos \
  --pos nagoya/run1=output/hybrid_nagoya1_multistage_m4_fixedpos_bridge05_vertical025_veld_vertical10_truthfree.pos \
  --pos nagoya/run2=output/nagoya2_selected_quality_rtkbaseline_tier2_truthfree.pos \
  --pos nagoya/run3=output/nagoya3_selected_quality_rtkbaseline_truthfree.pos \
  --output-json output/kf_fgo_selected_ppc_matrix.json \
  --markdown-output output/kf_fgo_selected_ppc_matrix.md
```

The resulting distance-weighted official score is **78.716546%**. Tokyo 1 is
**84.631656% FIX**. The separate Nagoya 1 FIX-target profile is generated by
`gnss ppc-demo` with the `goal_kf_current_r2_min8_rate20_rescue29_8` settings
recorded in its summary JSON; it measures **85.100974% FIX**, **0.912712%
Wrong FIX/FIX**, and **1.459572 m P95 H**. It is intentionally not substituted
into the six-run official-score matrix.
...
MPLBACKEND=Agg python3 scripts/generate_ppc_goal_scorecard.py \
  --lib-matrix output/kf_fgo_selected_ppc_matrix.json \
