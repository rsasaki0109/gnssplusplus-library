...
| [Google Smartphone Decimeter Challenge](https://www.ion.org/gnss/googlecompetition.cfm) | standalone-signoff | phone-grade stress | precise ground truth for raw GNSS and IMU traces | Android raw GNSS measurements and sensor logs | `smartphone-gnss-workflow` runs frozen archive validation, GPS L1 RINEX mapping, native SPP, truth sign-off, and bundle generation | phone antenna/clock behavior is a different receiver class; the precise-product variant regressed and is not promoted |
