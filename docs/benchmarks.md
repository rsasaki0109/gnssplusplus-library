...
| [Google Smartphone Decimeter Challenge](https://www.ion.org/gnss/googlecompetition.cfm) | adapter-contract | phone-grade stress | precise ground truth for raw GNSS and IMU traces | Android raw GNSS measurements and sensor logs | `smartphone-gnss-adapter` preserves and validates GSDC 2023 raw rows; native solver mapping remains pending | useful stress data, but phone antenna/clock behavior is a different receiver class |
