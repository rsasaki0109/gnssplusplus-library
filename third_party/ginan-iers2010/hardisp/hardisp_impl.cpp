// libgnss++ shim: include path adjusted (header co-located after
// vendoring) and `GTime epoch` parameters replaced with `double mjd_utc`
// throughout the HARDISP entry point + admint callees.
...
int iers2010::hisp::hardisp_impl(int irnt, double samp, double tamp[3][ntin],
  ...
  iers2010::hisp::admint(amp, phase, mjd_utc, az, f, pz, ntin, ntout);

  for (int i = 0; i < ntin; i++) {
    amp[i] = tamp[1][i];
    phase[i] = tph[1][i];
  }
  iers2010::hisp::admint(amp, phase, mjd_utc, aw, f, pw, ntin, ntout);

  for (int i = 0; i < ntin; i++) {
    amp[i] = tamp[2][i];
    phase[i] = tph[2][i];
  }
  iers2010::hisp::admint(amp, phase, mjd_utc, as, f, ps, ntin, ntout);
